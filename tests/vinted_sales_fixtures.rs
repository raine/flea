...
use flea::{
    AppError, PortalId,
    dependencies::{
        ApplicationDependencies, VintedCredentialRecord, VintedSalesApi, VintedSalesRequest,
    },
    domain::vinted_sale::VintedSalesStatus,
...
#[derive(Default)]
struct FixtureApi {
    calls: Mutex<Vec<VintedSalesRequest>>,
}
...
fn credentials() -> VintedCredentialRecord {
    VintedCredentialRecord::new_for_adapter(
        ...
        Some("private-login".into()),
        "private-access".into(),
        "private-refresh".into(),
        u64::MAX,
        "private-device".into(),
        "private-anonymous".into(),
        None,
    )
}
...
fn cli_defaults_and_pagination_preserve_transaction_identity_and_redact_extras() {
    ...
    let result = run_with_dependencies(
        ["flea", "--format", "json", "vinted", "sales", "list"],
        &dependencies,
    );
    ...
    assert_eq!(
        envelope["data"]["sales"][1]["transaction_user_status"],
        "future_status"
    );
    assert_eq!(envelope["data"]["next_page"], 2);
    assert_eq!(
        envelope["next_actions"][0]["command"],
        "flea vinted --portal fi sales list --status completed --page 2 --limit 20"
    );
    for absent in [
        "private-",
        "listing_id",
        ...
        assert!(
            ...
            "unexpected field: {absent}"
        );
    }
    assert_eq!(
        *api.calls.lock().unwrap(),
        vec![VintedSalesRequest::default()]
    );

    let result = run_with_dependencies(
        [
            "flea", "--format", "json", "vinted", "sales", "list", "--page", "2", "--limit", "2",
            "--status", "all",
        ],
        &dependencies,
    );
...
fn toon_and_all_status_filters_use_the_same_read_path() {
    for (status, expected) in [
        ("all", VintedSalesStatus::All),
        ("in_progress", VintedSalesStatus::InProgress),
        ("completed", VintedSalesStatus::Completed),
        ("canceled", VintedSalesStatus::Canceled),
    ] {
        let (dependencies, api) = setup();
        let result = run_with_dependencies(
            [
                "flea", "--format", "toon", "vinted", "sales", "list", "--status", status,
            ],
            &dependencies,
        );
        assert_eq!(result.exit_code, 0, "{}", result.document);
...
fn invalid_inputs_do_not_access_credentials_or_orders() {
    ...
    for args in [
        ...
        let result = run_with_dependencies(
            [
                "flea", "--format", "json", "vinted", "sales", "list", args[0], args[1],
            ],
            &dependencies,
        );
