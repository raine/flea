...
fn authenticated_sales_history_has_a_bounded_page_and_safe_identity() {
    assert_eq!(
        std::env::var("FLEA_LIVE_VINTED_SALES_SMOKE").as_deref(),
        Ok("1"),
        "set FLEA_LIVE_VINTED_SALES_SMOKE=1 to acknowledge a read-only account request"
    );
    let output = Command::new(env!("CARGO_BIN_EXE_flea"))
        .args([
            "--format", "json", "vinted", "sales", "list", "--limit", "1",
        ])
        .output()
    ...
    assert!(
        ...
        "sales request must succeed; account output withheld"
    );
